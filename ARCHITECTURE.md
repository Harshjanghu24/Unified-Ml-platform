...
| High-cardinality drop | `preprocessor.py` | Drop categorical cols with >20 unique values |
| ID column detection | `preprocessor.py` | Drop columns with 100% unique + "id"/"key" in name |
| Feature explosion cap | `preprocessor.py` | Truncate to 500 features after OHE |
| SHAP subsampling | `explainability.py` | Max 100 test samples, 200 background samples |
| EDA column limit | `eda.py` | Max 8 columns for histograms/box plots |
| Correlation heatmap limit | `eda.py` | Max 15 columns |
