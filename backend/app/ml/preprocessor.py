...
def preprocess_dataset_core(
    df: pd.DataFrame, target_column: str, problem_type: str = None, config: dict = None
) -> dict:
    """
        ...
        encoding_analysis.append(
            {
                ...
                "encoding": (
                    "One-Hot"
                    if strategy == "onehot"
                    else ("Label Encoding" if strategy == "label" else "Dropped")
                ),
            }
        )
    ...
    if selected_features is not None:
        ...
        _ = (working_df.shape[1] - 1) - (len(cols_to_keep) - 1)
    ...
    if missing_values_fixed > 0:
        if missing_value_method == "drop_cols":
            ...
            report["missing_values"] = (
                "Dropped all rows containing missing values in feature columns."
            )
        else:
            ...
            report["missing_values"] = (
                f"Missing values imputed using {missing_value_method.capitalize()} method."
            )
    else:
    ...
    if outlier_method == "iqr" and numeric_cols:
        ...
        report["outliers"] = (
            f"Outliers removed using Z-score method ({outliers_removed} rows removed)."
        )
    else:
    ...
    if categorical_cols:
        ...
        label_cols_to_apply = [
            col for col in categorical_cols if column_strategies.get(col) == "label"
        ]
    ...
    if scaling_method != "none" and numeric_cols:
        ...
        if scaler:
            ...
            if existing_numeric_cols:
                working_df[existing_numeric_cols] = scaler.fit_transform(
                    working_df[existing_numeric_cols]
                )
                report["scaling"] = (
                    f"Applied {scaling_method.capitalize()} scaling to numeric features."
                )
    else:
    ...
    # 9. Target Encoding
    target_encoder = None
    target_is_non_numeric = not pd.api.types.is_numeric_dtype(working_df[target_column])
    if problem_type in ("binary", "multiclass") or (problem_type is None and target_is_non_numeric):
        target_encoder = LabelEncoder()
        working_df[target_column] = target_encoder.fit_transform(working_df[target_column])
        report["target_encoding"] = (
            f"Target label encoded. Classes: {list(target_encoder.classes_)}"
        )
    else:
    ...
    processing_summary = {
        ...
        "encoding_strategy_used": (
            "Manual Custom"
            if config.get("column_encodings")
            else "Adaptive (One-Hot <=20, Label <=1000, Drop >1000)"
        ),
    }
...
def build_preprocessing_pipeline(df: pd.DataFrame, target_column: str, problem_type: str = None):
    ...
    dropped_by_engine = {
        col for col, strat in res.get("column_strategies", {}).items() if strat == "drop"
    }
    ...
    X_raw = df.drop(columns=[target_column] + cols_to_drop_from_raw)
